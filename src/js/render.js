function homeView() {
  ...
  const articleMore = (!state.articlesExpanded && articles.length > 12)
    ? `<button class="more">${en ? `Show all ${articles.length} articles` : `展开全部 ${articles.length} 篇`} →</button>` : '';
  ...
  const productList = products.map((p, i) => {
    ...
    const statusText = localize(p, 'status');
    const status = statusText ? `<span class="status">${escapeHtml(statusText)}</span>` : '';
    const linkHtml = link
      ? `<a class="plink" href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(localize(link, 'text'))} ↗</a>`
      : '';
    const descText = localize(p, 'description');
  ...
  return `
    ...
    <div class="wrap mode-${state.mode}">
      <header class="hero">
        <div class="idrow">
          <button class="avatar-btn" id="avatarBtn" type="button" title="${en ? 'Switch Products / Articles' : '点击切换 产品 / 文章'}" aria-label="${en ? 'Switch Products / Articles' : '点击头像切换 产品 / 文章'}">
        ...
        <p class="about products">${en ? 'Open-source enthusiast.' : '狂热的开源玩家。'}</p>
        <p class="about articles">${en ? 'I chose the road less traveled, and that has made all the difference.' : '我的前面有两条路，我选择了人迹更少的道路，因此生命迥然不同。'}</p>
      </header>

      <section class="block block-products">
        <h2 class="stitle">${en ? 'Products' : '「产品」'}</h2>
        <div class="products">${productList}</div>
      </section>

      <section class="block block-articles">
        <h2 class="stitle">${en ? 'Articles' : '「文章」'}</h2>
      ...
      <footer>
        ...
        <span class="social">
          ...
          <button class="lang-switch" id="langBtn" type="button" aria-label="${en ? '切换为中文' : 'Switch to English'}">${en ? '中文' : 'EN'}</button>
        </span>
      </footer>
    </div>`;
}
